...
EXTENSION_NAME = "My Surface Gripper"
...
class Extension(omni.ext.IExt):
    ...
    def assembleRobot2(self):
        robot_assembler = RobotAssembler()
        base_robot_path = "/World/ur10e_2"
        attach_robot_path = "/World/MODE3_Gripper"
        # robot_assembler.convert_prim_to_rigid_body(attach_robot_path)
        # UsdPhysics.CollisionAPI.Apply(attach_robot_path)
        # # if mass > 0:
        # massAPI = UsdPhysics.MassAPI.Apply(attach_robot_path)
        # massAPI.CreateMassAttr(4.0)
        # UsdPhysics.RigidBodyAPI.Apply(attach_robot_path)
        # UsdPhysics.CollisionAPI(attach_robot_path)
        base_robot_mount_frame = "/tool0"
        attach_robot_mount_frame = ""
    ...
    def convertToRigidBody(self, bodyType, boxActorPath, mass, scale, position, rotation, color):
        p = Gf.Vec3f(position[0], position[1], position[2])
        orientation = Gf.Quatf(rotation[0], rotation[1], rotation[2], rotation[3])
        scale = Gf.Vec3f(scale[0], scale[1], scale[2])
        bodyGeom = bodyType
